\DCM{TODO: The treatment of the seeded electron chains that collected some luminosity before the unseeded chains came online.} %
\\
\DCM{TODO: Tone down the self-congratulatory language coming from this writeup originally having been written for a PhD progress evaluation committee.} %
The prescales of triggers are not simulated in the MC sample - mainly not to lose statistics but also because the anticipated prescales and the actual prescales during data-taking would often be different. %
Thus, event-by-event weights need to be applied to the events in the MC samples to mimic the effect of the prescales in the data. %
The usual approach adopted in ATLAS to perform such reweighting is outlined in \cite{Buttinger:2014726}, however, this approach assumes that none of the trigger chains that are being used to collect data have overlapping L1 items. %
The defining characteristic of the unseeded electron trigger chains, described in detail in \ref{subsec:trigger-for-electron}, was to seed the HLT item of the trigger chain on any active L1 item in the entire trigger menu of ATLAS, rather than seeding the HLT item on just one L1 item as it is usually done otherwise. %
Thus, special care is needed in using them in the offline analysis. %
In particular, since both of the electron trigger chains share all of the L1 items, the interactions between the prescales of the L1 items as well as the prescales of the HLT items themselves need to be carefully considered while correcting the MC to reflect the prescales applied during data-taking.%  
\subsection{The Issue of Correlated Prescales}
\label{subsec:issue-of-correlated-prescales}
The basic rules for combining prescales of triggers, as outlined in \cite{Buttinger:2014726}, are as follows:
\begin{itemize}
    \item The effective prescale $P_Z$ of a logical AND of two triggers $X,Y$ is given by
    %\DM{Relevant for a full trigger chain which needs to be treated as the logical AND of the L1 and the HLT parts. This point that in the context of prescale reweighting, treating the L1 and the HLT parts separately is important because the prescales for them are applied separatedly is emphasized in the appendix.}
    \begin{align}
    \label{eq:buttinger-and}
    \frac{1}{P_Z}=\frac{1}{P_X}\cdot\frac{1}{P_Y}
    \end{align}
    \item The effective prescale $P_Z$ of a logical OR of two triggers $X, Y$ is given by 
    \begin{align}
    \label{eq:buttinger-or}
    \frac{1}{P_Z}=1-\bigg(1-\frac{1}{P_X}\bigg)\bigg(1-\frac{1}{P_Y}\bigg)
    \end{align} 
\end{itemize}
where $P_{X,Y}$ are the prescales of the two triggers $X,Y$. %
Notice that while analyses usually (if not always) employ a logical OR of trigger chains to collect data, the logical structure of a trigger chain itself is a logical AND between the HLT part and the L1 part of the chain. Thus, the rule for combining prescales of triggers in a logical AND is always relevant. %
It is intuitively clear that these rules can only be applied if the enactment of prescales is independent between the triggers $X$ and $Y$. %
For example, consider the following two stylized trigger chains:  
\begin{itemize}
    \item $X=\texttt{HLT\_e60\_L1\_EM22}$ (a trigger requiring an EM L1 object (an electron or a photon) with $p_{\textrm T}>22$ GeV and an HLT electron with $p_{\textrm T}>60$ GeV)
    \item $Y=\texttt{HLT\_2e60\_L1\_EM22}$ (a trigger requiring two EM L1 objects with $p_{\textrm T}>22$ GeV and an HLT electron with $p_{\textrm T}>60$ GeV)
\end{itemize}
If $\texttt{L1\_EM22}$ is prescaled at $2$ and the HLT parts of both the chains are running unprescaled, the effective prescale for $X\ \vert \vert\ Y$ is simply $2$ since there is a $50\%$ probability that  $X\ \vert \vert\ Y$ is ``prescale-wise online'' -- corresponding to the $50\%$ probability that $\texttt{L1\_EM22}$ is ``prescale-wise online''\footnote{Since the prescales are applied \emph{after} running the triggers for L1 triggers, it might be slightly misleading to use the phrase ``prescale-wise online'', however, hopefully, the intended meaning is communicated.}. %
However, naively applying \eqref{eq:buttinger-or} will lead one to conclude that the effective prescale of $X\ \vert\vert\ Y$ should be $4/3$. %
Notice that the effective prescale of $X\ \vert\vert\ Y$ would indeed have been $4/3$ if, for example, $Y$ had been $\texttt{HLT\_2e60\_L1\_EM18}$ with $\texttt{L1\_EM18}$ prescaled at $2$ and everything else unchanged. %
Thus, it is clear that we need to be careful whenever we are dealing with HLT chains that, either fully or partially, share L1 seeds. %
In the electron channel of the analysis, we collect data using the logical OR combination of the HLT chains $\texttt{HLT\_e5\_lhvloose\_nod0\_bBeexM6000t}$ and $\texttt{HLT\_2e5\_lhvloose\_nod0\_bBeexM6000t}$. %
As mentioned earlier, both of these trigger chains are seeded on the entire L1 menu of Run~2 (with $\sim$~500 L1 items), i.e., fully share the L1 seeding. %
We thus cannot straightforwardly use \eqref{eq:buttinger-or} to deal with the logical OR of the HLT chains of interest. %
\\\\
We describe our approach to dealing with this issue in the next subsection \ref{subsec:app-prw-math}. %
However, to motivate the approach, we simply mention that the underlying idea is to recursively use the following basic probability rules on the trigger expression of interest:
\begin{align}
    P(X || Y)&=P(X)+P(Y)-P(X\cap Y)\\
    P(XY)&=P(X)\cdot P(Y),
\end{align}
where the second of these rules is true only if $X$ and $Y$ are independent. %
In the context of trigger expressions, the prescales of each of the HLT items as well as the prescales of each of the L1 items are mutually independent. %
Thus, for example, for some HLT items $A$, $B$ and an L1 item $X$, while $P(AX || BX)\neq (P(AX)+ P(BX ) -  P(AX )   P(BX )=P(A)P(X)+P(B)P(X)-P(A)P(B)P^2(X))$, 
\begin{align}
    P(AX || BX ) &= P(AX)+P(BX)-P(AXBX)\\
    &= P(AX)+P(BX)-P(ABX)\\
    &= P(A)P(X)+P(B)P(X)-P(A)P(B)P(X)
\end{align}
It is intuitively clear that the probability term $P(X)$ representing the prescale weight of a trigger $X$ should be $1/P_X$ where $P_X$ is the prescale of the trigger $X$. While we are using the letter $X$ here with an ambiguous meaning (namely, it's used both as a boolean variable representing a trigger decision and as some sort of a set such that $P(X)$ is a meaningful term), the precise correspondence between the boolean variables representing trigger decisions and the corresponding sets of events that are relevant to the discussion of prescale weights is established in the following. %
\subsection{Solution to the Issue of Correlated Prescales}
\label{subsec:app-prw-math}
%We use boolean variables to represent the ``prescale enaction'' for a given trigger combination. %
To make the discussion unambiguous, we first introduce (or recall) a few definitions and some notation, along with deriving useful mathematical identities.
\subsubsection{Preliminary Definitions, Notation, and Algebraic Identities}
\label{subsubsec:app-prw-math-defs-and-identities}
...
\subsubsection{Evaluation of the Prescale Weights}
\label{subsubsec:app-prw-evaluation-of-prescale-weights}
...
\\\\
\textbf{Rule $1$: $w^i_{\mathcal{A}+\mathcal{B}}=w^i_{\mathcal{A}}+w^i_{\mathcal{B}}-w^i_{\mathcal{AB}}$.} Utilizing the composition rules \eqref{eq:composition-set-operations-on-slashed-u} for the reduced unprescaled sets, we can write 
\begin{subequations}
...
\end{align}
\end{subequations}
\textbf{Rule $2$: Iff the enactment of prescales is independent between $\mathcal{A}$ and $\mathcal{B}$, $w^i_{\mathcal{A}\mathcal{B}}=w(\mathcal{A})w(\mathcal{B})$}. Again, utilizing the composition rules \eqref{eq:composition-set-operations-on-slashed-u} for the reduced unprescaled sets, we can write
\begin{align}
...
%\end{align}
\subsubsection{Evaluation of the Prescale Weights for the Unseeded \texttt{BeeX} Triggers}
\label{subsubsec:app-prw-evaluation-of-prescale-weights-for-beex}
