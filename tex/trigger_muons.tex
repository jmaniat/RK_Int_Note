...
In the level-1 trigger (L1), it requires two muon regions of interest (RoI) with $p_T > 6$~GeV and opening angle $\Delta R < 1.5$.
In the level-2 trigger, it requires a di-muon vertex with $\chi^2 < 20$ and $0.1 < m_{\mu\mu} < 6.5$~GeV,
two opposite-charged tracks with mass in range $0.1 - 15$~GeV and fit to a common vertex with $\chi^2 < 20$.
In the Event Filter, the $B_d \rightarrow K^{*0} \mu^+ \mu^-$ vertex is reconstructed with the requirements
of $B_d$-vertex $\chi^2 < 60$, $K^{*0}$-vertex $\chi^2 < 400$, $m_{B_d} \in [4.6, 5.9]$~GeV and $m_{K^{*0}} \in [0.7, 1.1]$~GeV.

The trigger efficiency, defined as the ratio of events firing the trigger to those passing the offline selections,
is a crucial part of the total efficiency. For a normal di-muon trigger, the strategy is to factorise the efficiency as 
...
These selections are rather empirical and cannot entirely remove the candidates in which
the tag and probe muon is actually matched to the other legs, but the systematic effect
can be evaluated through the closure test and include as part of the systematic uncertainties.
